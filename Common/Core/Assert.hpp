...
#	define INTERNAL_ASSERT_IMPL(channel, check, msg, ...) do { if(!(check)) { SPDLOG_LOGGER_ERROR( ::Logging::GetLogger( Logging::Channel::channel ), msg, __VA_ARGS__ ); DEBUGBREAK(); } } while(0)
...
#	define NOT_IMPLEMENTED do { LOG_ERROR( Assert, "Function not implemented!" ); DEBUGBREAK(); } while(0)
#else
#	define ASSERT(x, ...) do {} while(0)
#	define ASSERT_CHANNEL(channel, x, ...) do {} while(0)

#	define NOT_IMPLEMENTED do { LOG_ERROR( Assert, "Function not implemented!" ); } while(0)
#endif
